...
def joint_power(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # compute the reward
    reward = torch.sum(
        torch.abs(wp.to_torch(asset.data.joint_vel)[:, asset_cfg.joint_ids] * wp.to_torch(asset.data.applied_torque)[:, asset_cfg.joint_ids]),
        dim=1,
    )
    return reward
...
def joint_pos_penalty(
    ...
    running_reward = torch.linalg.norm(
        (wp.to_torch(asset.data.joint_pos)[:, asset_cfg.joint_ids] - wp.to_torch(asset.data.default_joint_pos)[:, asset_cfg.joint_ids]), dim=1
    )
    reward = torch.where(
...
def wheel_vel_penalty(
    ...
    cmd = torch.linalg.norm(env.command_manager.get_command(command_name), dim=1)
    body_vel = torch.linalg.norm(wp.to_torch(asset.data.root_lin_vel_b)[:, :2], dim=1)
    joint_vel = torch.abs(wp.to_torch(asset.data.joint_vel)[:, asset_cfg.joint_ids])
...
def joint_mirror(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, mirror_joints: list[list[str]]) -> torch.Tensor:
    ...
    for joint_pair in env.joint_mirror_joints_cache:
        ...
        diff = torch.sum(
            torch.square(wp.to_torch(asset.data.joint_pos)[:, joint_pair[0][0]] - wp.to_torch(asset.data.joint_pos)[:, joint_pair[1][0]]),
            dim=-1,
        )
        reward += diff
...
def feet_distance_y_exp(
    ...
    asset: RigidObject = env.scene[asset_cfg.name]
    cur_footsteps_translated = wp.to_torch(asset.data.body_link_pos_w)[:, asset_cfg.body_ids, :] - wp.to_torch(asset.data.root_link_pos_w)[
        :, :
    ].unsqueeze(1)
    ...
    for i in range(n_feet):
        footsteps_in_body_frame[:, i, :] = math_utils.quat_apply(
            math_utils.quat_conjugate(wp.to_torch(asset.data.root_link_quat_w)), cur_footsteps_translated[:, i, :]
        )
...
def feet_height_body(
    ...
    """Reward the swinging feet for clearing a specified height off the ground"""
    asset: RigidObject = env.scene[asset_cfg.name]
    cur_footpos_translated = wp.to_torch(asset.data.body_pos_w)[:, asset_cfg.body_ids, :] - wp.to_torch(asset.data.root_pos_w)[:, :].unsqueeze(1)
    footpos_in_body_frame = torch.zeros(env.num_envs, len(asset_cfg.body_ids), 3, device=env.device)
    cur_footvel_translated = wp.to_torch(asset.data.body_lin_vel_w)[:, asset_cfg.body_ids, :] - wp.to_torch(asset.data.root_lin_vel_w)[
        :, :
    ].unsqueeze(1)
    footvel_in_body_frame = torch.zeros(env.num_envs, len(asset_cfg.body_ids), 3, device=env.device)
    for i in range(len(asset_cfg.body_ids)):
        footpos_in_body_frame[:, i, :] = math_utils.quat_apply_inverse(
            wp.to_torch(asset.data.root_quat_w), cur_footpos_translated[:, i, :]
        )
        footvel_in_body_frame[:, i, :] = math_utils.quat_apply_inverse(
            wp.to_torch(asset.data.root_quat_w), cur_footvel_translated[:, i, :]
        )
...
def feet_slide(
    ...
    # feet_vel = wp.to_torch(asset.data.body_lin_vel_w)[:, asset_cfg.body_ids, :2]
...
def base_height_l2(
    ...
    if sensor_cfg is not None:
        ...
        if torch.isnan(ray_hits).any() or torch.isinf(ray_hits).any() or torch.max(torch.abs(ray_hits)) > 1e6:
            adjusted_target_height = wp.to_torch(asset.data.root_link_pos_w)[:, 2]
        else:
    ...
    reward = torch.square(wp.to_torch(asset.data.root_pos_w)[:, 2] - adjusted_target_height)
    reward *= torch.clamp(-wp.to_torch(env.scene["robot"].data.projected_gravity_b)[:, 2], 0, 0.7) / 0.7
    return reward
...
def lin_vel_z_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    reward = torch.square(wp.to_torch(asset.data.root_lin_vel_b)[:, 2])
    reward *= torch.clamp(-wp.to_torch(env.scene["robot"].data.projected_gravity_b)[:, 2], 0, 0.7) / 0.7
    return reward
...
def ang_vel_xy_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    reward = torch.sum(torch.square(wp.to_torch(asset.data.root_ang_vel_b)[:, :2]), dim=1)
    reward *= torch.clamp(-wp.to_torch(env.scene["robot"].data.projected_gravity_b)[:, 2], 0, 0.7) / 0.7
    return reward
...
def flat_orientation_l2(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    reward = torch.sum(torch.square(wp.to_torch(asset.data.projected_gravity_b)[:, :2]), dim=1)
    reward *= torch.clamp(-wp.to_torch(env.scene["robot"].data.projected_gravity_b)[:, 2], 0, 0.7) / 0.7
    return reward
...
def track_lin_vel_x_exp(
    ...
    """Reward tracking of longitudinal velocity command using exponential kernel."""
    asset: RigidObject = env.scene[asset_cfg.name]
    vx_cmd = env.command_manager.get_command(command_name)[:, 0]
    vx_actual = wp.to_torch(asset.data.root_lin_vel_b)[:, 0]
...
def track_roll_rate_exp(
    ...
    roll_rate_actual = wp.to_torch(asset.data.root_ang_vel_b)[:, 0]
