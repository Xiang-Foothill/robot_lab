...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # remove random pushing (attribute names differ across env configs)
    ...
    if args_cli.keyboard:
        ...
        config = Se2KeyboardCfg(
            v_x_sensitivity=getattr(ranges, "lin_vel_x", getattr(ranges, "lin_acc_x", (0.0, 1.0)))[1],
            v_y_sensitivity=getattr(ranges, "lin_vel_y", (0.0, 0.0))[1],
            omega_z_sensitivity=getattr(ranges, "ang_vel_z", (0.0, 1.0))[1],
        )
    ...
    if version.parse(installed_version) >= version.parse("4.0.0"):
        ...
        # rsl_rl < 4.0.0: extract policy_nn manually
